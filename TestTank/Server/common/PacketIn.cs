...
public static class PacketInPool
{
    ...
    {
        if (!Pool.TryTake(out var packet)) return new PacketIn();
    ...
    {
        if (Interlocked.Increment(ref _currentPoolSize) <= MaxSize)
        {
            Pool.Add(packet);
        }
        else
...
public class PacketIn : SimplePacket
{
    ...
    private object Deserialize(Type type)
    {
        ...
        foreach (var field in
                 type.GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)) // 拆解可序列化的类
        {
